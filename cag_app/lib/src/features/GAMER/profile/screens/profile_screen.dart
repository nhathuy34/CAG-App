...
class ProfileScreen extends ConsumerWidget {
    ...
    return Column(
      children: [
        Container(
          ...
          child: Padding(
            padding: EdgeInsets.only(left: Responsive.widthPercent(context, 5), right: Responsive.widthPercent(context, 5), top: Responsive.heightPercent(context, 6)),
            child: Column(
              children: [
  ...
  Widget _avataHeader(BuildContext context, AsyncValue<Usermodel> userProfileAsync) {
    return Row(
      ...
      children: [
        _buildAvatarStack(context, userProfileAsync),
        SizedBox(width: Responsive.widthPercent(context, 4)),
        Expanded(
          child: userProfileAsync.when(
            data: (user) => _buildUserInfo(context, user.username , user.id.toString()), 
            loading: () => const CircularProgressIndicator(color: AppTheme.cyanNeon),
            error: (err, stack) => _buildUserInfo(context, "Lỗi tải dữ liệu", "N/A"),
          ),
        ),
      ],
    );
  ...
  Widget _buildAvatarStack(BuildContext context, AsyncValue<Usermodel> userProfileAsync) {
    ...
    return Stack(
      ...
      children: [
        Container(
          width: Responsive.widthPercent(context, 18),
          height: Responsive.widthPercent(context, 18),
              ...
              ? ClipRRect( // Nên dùng ClipRRect để bo góc ảnh cho đẹp
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (ctx, error, stackTrace) => Center(
                      child: Icon(Icons.person, size: Responsive.fontSize(context, 40), color: Colors.white54),
                    ),
                  ),
                )
              : Center( 
        ...
        Positioned(
          bottom: -6,
          right: -10,
          child: _buildBadge(context, 'LVL.8', color: AppTheme.cyanNeon, isLevel: true),
        ),
      ],
    );
  }
  ...
  Widget _buildUserInfo(BuildContext context, String fullName, String userId) { 
    ...
    return Column(
      ...
      children: [
        ...
        Text('ID: #$displayId', style: GoogleFonts.rajdhani(color: AppTheme.textDim, fontSize: Responsive.fontSize(context, 14))),
        
        const SizedBox(height: 8),
        Row(
          children: [
            _buildBadge(context, 'BRONZE'),
  ...
  // --- WIDGET BADGE CHUNG ---
  Widget _buildBadge(BuildContext context, String label, {Color color = Colors.white70, bool isLevel = false}) {
    return Container(
      ...
      child: Text(label, style: GoogleFonts.rajdhani(color: color, fontSize: Responsive.fontSize(context, 12), fontWeight: FontWeight.bold)), 
    );
  }
}
