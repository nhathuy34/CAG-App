...
class CommuTabBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SliverPersistentHeader(
      pinned: true, // Tương đương 'sticky top-0'
      delegate: _SliverAppBarDelegate(
        child: ClipRect( // Giới hạn vùng Blur
          child: BackdropFilter(
            filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10), // backdrop-blur
            child: Container(
              padding: const EdgeInsets.only(top: 8), // pt-2
              decoration: BoxDecoration(
                color: const Color(0xFF020617).withOpacity(0.95), // bg-slate-950/95
                border: const Border(
                  bottom: BorderSide(color: Color(0xFF334155), width: 1), // border-slate-700
                ),
              ),
  ...
  Widget _buildTab({
    ...
    return InkWell(
      onTap: () {
        if (needsAuth) {
          // triggerAuth("Đăng nhập để xem danh sách theo dõi.");
        }
